...
export class CartPage extends BasePage {
  ...
  get itemQuantities() { return this.page.locator('.cart_quantity') }
  ...
  async checkoutHealed() {
    await this.clickHealed(
      this.checkoutButton,
      'Checkout button — proceeds to the shipping form',
    )
    await this.page.waitForURL('/checkout-step-one.html')
  }

  async continueShoppingHealed() {
    await this.clickHealed(
      this.continueShoppingButton,
      'Continue Shopping button — returns to the product catalog',
    )
    await this.page.waitForURL('/inventory.html')
  }
}